...
* Currently, the pipeline is triggered by a push to master since it is a very simple application. If the app was more complicated, I would aim to create a multibranch pipeline where the build and all necessary tests would be kicked off by the creation of a pull request. Only then, after all tests were successful, on push to master, I would deploy to Docker Hub.
